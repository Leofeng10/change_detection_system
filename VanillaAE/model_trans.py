...
class VisionTransformer(nn.Module):
    def __init__(self, opt):
        ...
        # generator
        self.netG = Trans().to(self.device)
    ...
    def gen_update(self, data, epoch, matrix=None):
        ...
        errG_pix = self.criterion(recon, real[:, frame]) * self.opt.mse_w
        errG_pix = errG_pix.float().cuda()
        # apply focal frequency loss
        if epoch >= self.opt.freq_start_epoch:
            errG_freq = self.criterion_freq(recon, real[:, frame], matrix)
        else:
