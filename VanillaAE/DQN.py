...
#
#             next_state, reward, uav_done, info, _, _, _, _, _, _ = env.step(action.detach(),i)
#
#             total_reward += reward
#
...
#
#     # check_point_Qlocal=torch.load('C:/Users/Feng Zhunyi/Desktop/change_detection/scripts/Qlocal.pth')
#     # check_point_Qtarget=torch.load('C:/Users/Feng Zhunyi/Desktop/change_detection/scripts/Qtarget.pth')
...
#     np.savetxt('VanillaAE/scores.txt', scores, fmt='%d')
#     np.savetxt('VanillaAE/avg_scores.txt', scores, fmt='%d')
#
...
def run_episode(env, eps):
    ...
    while (1):
        ...
        if count % 5 == 0 and len(env.replay_memory) > BATCH_SIZE:
            # batch = env.replay_memory.sample(BATCH_SIZE)
            env.le.arn(gamma, BATCH_SIZE)  # 训练Q网络
        if n_done >= env.n_uav:
            break
        # plt.pause(0.001)
    if success_count >= 0.8 * env.n_uav and env.level < 10:
...
def train():
    ...
    for i_episode in range(num_episodes):
        ...
        if i_episode % print_every == 0 and i_episode > 0:
            print(
                'sum_Episode: {:5} Episode: {:5} Score: {:5}  Avg.Score: {:.2f}, eps-greedy: {:5.2f} Time: {:02}:{:02}:{:02} level:{:5}  num_success:{:2}  num_crash:{:2}  num_none_energy:{:2}  num_overstep:{:2}'. \
                format(i_episode + epoch, i_episode, score, avg_score, eps, dt // 3600, dt % 3600 // 60, dt % 60,
                       env.level, info[0], info[1], info[2], info[3]))
        # 保存模型参数
        if i_episode % 100 == 0:
            # 每100周期保存一次网络参数
            state = {'model': env.q_target.state_dict(), 'optimizer': env.optim.state_dict(),
...
#     import matplotlib.pyplot as plt
#
#     # Step 1: Read numbers from the file and store them into an array
#     numbers = []
#     with open('VanillaAE/avg_scores.txt', 'r') as file:
#         for line in file:
#             # Convert each line to an integer (or float if your numbers are not integers)
#             numbers.append(float(line.strip()))
#
#     # Step 2: Plot the array
#     epochs = range(1, 40001)
#     plt.plot(epochs, numbers)
#     plt.title('Averaged total reward in training')
#     plt.xlabel('Epochs')
...
if __name__ == '__main__':
    ...
    with open('VanillaAE/scores.txt', 'w') as file:
        for item in scores:
            # Write each item on a new line
            file.write("%s\n" % item)
    with open('VanillaAE/avg_scores.txt', 'w') as file:
